    ...
    if (!estaActivo && seMuestraBurgerMenu)
        return (
            <nav className='w-full bg-transparent text-black flex flex-items justify-end gap-2 fixed top-0 z-50 animate__animated  animate_faster'>
                <svg onClick={() => { setEstaActivo(!estaActivo) }} xmlns="http://www.w3.org/2000/svg" fill="white" viewBox="0 0 24 24" stroke-width="1.5" stroke="white" className="w-12 p-2 hover:cursor-pointer"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" /></svg>
            </nav>
        )
    else
        return (
